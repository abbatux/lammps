...
double FixSMD_TLSPH_ReferenceConfiguration::memory_usage() {
	...
	bytes += nmax * maxpartner * sizeof(Vector3d); // sNormal array
	return bytes;
...
void FixSMD_TLSPH_ReferenceConfiguration::copy_arrays(int i, int j, int delflag) {
	...
	for (int m = 0; m < npartner[j]; m++) {
		...
		partnerx0[j][m] = partnerx0[i][m];
		partnervol[j][m] = partnervol[i][m];
	}
}
...
//printf("pack_exchange ...\n");
	...
	for (int n = 0; n < npartner[i]; n++) {
		...
		buf[m++] = partnerx0[i][n][0];
		buf[m++] = partnerx0[i][n][1];
...
//printf("nlocal=%d, nmax=%d\n", nlocal, nmax);
	...
	for (int n = 0; n < npartner[nlocal]; n++) {
		...
		partnerx0[nlocal][n][0] = static_cast<double>(buf[m++]);
		partnerx0[nlocal][n][1] = static_cast<double>(buf[m++]);
