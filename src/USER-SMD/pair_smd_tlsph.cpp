...
PairTlsph::PairTlsph(LAMMPS *lmp) :
	...
	comm_forward = 27; // this pair style communicates 20 doubles to ghost atoms : PK1 tensor + F tensor + damage_increment + npartner + flowstress_slope
...
void PairTlsph::compute(int eflag, int vflag) {
	...
	if (atom->nmax > nmax) {
		...
		delete[] stress_at_failure_init;
		stress_at_failure_init = new double[nmax];
		delete[] rSqMin;
	...
	for (i = 0; i < nlocal; i++) {
		...
		if (setflag[itype][itype] == 1) {
			if (mol[i] > 0) { // only do the following if particle has not failed -- mol < 0 means particle has failed
				...
				particle_dt[i] = (1.0-damage[i])*sqrt(rSqMin[i]) / ((1.0 + 1.2*Lookup[VISCOSITY_Q1][itype])*p_wave_speed + 1.2*Lookup[VISCOSITY_Q2][itype]*max_mu_ij[i] + sqrt(vijSq_max[i])); // Monaghan deltat_2
				if (particle_dt[i] < 1e-9) {
...
void *PairTlsph::extract(const char *str, int &i) {
  //  printf("in PairTlsph::extract\n");
	...
	} else if (strcmp(str, "smd/tlsph/stress_at_failure_init") == 0) {
		return (void *) stress_at_failure_init;
...
int PairTlsph::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc) {
	...
	for (i = 0; i < n; i++) {
		...
		buf[m++] = stress_at_failure_init[j]; //27
...
//sigma_dev_rate = sigmaInitial_dev - Deviator(sigmaFinal_dev);
	...
	case STRENGTH_LINEAR_PLASTIC:
		if (failureModel[itype].failure_gtn) {
		  ...
		  if (damage[i] == 0) {
		    ...
		    yieldStress = stress_at_failure_init[i];
		  }
	...
	case STRENGTH_JOHNSON_COOK:
		JohnsonCookStrength(Lookup[SHEAR_MODULUS][itype], Lookup[HEAT_CAPACITY][itype], mass_specific_energy, Lookup[JC_A][itype],
				    ...
				    sigmaInitial_dev, d_dev, sigmaFinal_dev, sigma_dev_rate, plastic_strain_increment, damage[i], stress_at_failure_init[i]);
		break;
	case STRENGTH_NONE:
